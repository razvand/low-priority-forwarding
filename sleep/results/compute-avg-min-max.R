...
cat("sleep", "cache-misses", mean(cm), min(cm), max(cm), sd(cm), 100*sd(cm)/mean(cm), sep=","); cat("\n")
cat("sleep", "running-time", mean(rt), min(rt), max(rt), sd(rt), 100*sd(rt)/mean(rt), sep=","); cat("\n")
